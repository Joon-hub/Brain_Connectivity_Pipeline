...
Train One-vs-One (OvO) logistic regression separately for left and right hemispheres.
...
Usage:
    ...
    python scripts/hemisphere/04_train_hemisphere_ovo.py --hemisphere both
    
    # With task testing:
    python scripts/hemisphere/04_train_hemisphere_ovo.py --hemisphere left --test_on_task
"""
...
def setup_logging(output_dir: Path, hemisphere: str) -> logging.Logger:
    ...
    log_file = output_dir / f"training_{hemisphere}_hemisphere_ovo.log"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Train hemisphere-specific One-vs-One logistic regression'
    )
...
def test_on_task_data(
    ...
    This reveals how well the OvO model generalizes from resting-state to task-based connectivity.
    ...
    logger.info("="*80)
    logger.info(f"Training final OvO model on ALL resting-state data")
    ...
    # =========================================================================
    # STEP 3: Train final OvO model on ALL rest data
    # =========================================================================
    
    logger.info("\nStep 3: Training final OvO model on ALL resting-state data...")
    ...
    logger.info(f"  OvO model trained in {train_time:.2f}s")
    ...
    # Note: OneVsOneClassifier doesn't have predict_proba in sklearn
    # We'll compute it manually by aggregating binary classifier votes
    try:
        y_task_proba = final_model.decision_function(X_task_scaled)
    except:
        y_task_proba = None
    ...
    if y_task_proba is not None:
        np.save(task_output_dir / 'task_decision_function.npy', y_task_proba)
    ...
    plot_confusion_matrix(
        ...
        title=f'{hemisphere.capitalize()} - Task Testing Confusion Matrix (OvO)'
    )
    
    plot_per_region_accuracy(
        per_region_metrics=task_per_region,
        save_path=task_output_dir / 'task_per_region_accuracy.png',
        title=f'{hemisphere.capitalize()} - Task Testing Per-Region Accuracy (OvO)'
    )
    
    plot_network_accuracy(
        network_metrics=task_network,
        save_path=task_output_dir / 'task_network_accuracy.png',
        title=f'{hemisphere.capitalize()} - Task Testing Network Accuracy (OvO)'
    )
    ...
    logger.info("\n" + "="*80)
    logger.info("TASK TESTING RESULTS (ONE-VS-ONE)")
    logger.info("="*80)
...
def train_single_hemisphere(
    ...
    """
    Train One-vs-One model for a single hemisphere.
    ...
    7. [If test_on_task] Train final OvO model on all rest data → Test on task data
    """
    
    logger.info(f"\n{'='*80}")
    logger.info(f"TRAINING {hemisphere.upper()} HEMISPHERE (ONE-VS-ONE)")
    logger.info(f"{'='*80}\n")
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"CROSS-VALIDATION WITH FOLD-WISE SCALING (ONE-VS-ONE)")
    logger.info(f"{'='*80}")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups=groups)):
        ...
        # Train OvO model
        logger.info(f"  Training OvO with C={C:.6f}, solver={solver}...")
        ...
        # Get decision function (for OvO, this is pairwise voting)
        try:
    ...
    if len(all_decision_functions) > 0:
        all_decision_functions = np.vstack(all_decision_functions)
    else:
        all_decision_functions = None
    ...
    overall_metrics = compute_classification_metrics(
        ...
        y_proba=all_decision_functions
    )
    ...
    logger.info(f"\nOVERALL RESULTS ({hemisphere.upper()} HEMISPHERE - ONE-VS-ONE):")
    ...
    plot_confusion_matrix(
        ...
        title=f'{hemisphere.capitalize()} Hemisphere - Confusion Matrix (OvO)'
    )
    
    plot_per_region_accuracy(
        per_region_metrics=per_region_metrics,
        save_path=output_dir / 'per_region_accuracy.png',
        title=f'{hemisphere.capitalize()} Hemisphere - Per-Region Accuracy (OvO)'
    )
    
    plot_network_accuracy(
        network_metrics=network_metrics,
        save_path=output_dir / 'network_accuracy.png',
        title=f'{hemisphere.capitalize()} Hemisphere - Network-Level Accuracy (OvO)'
    )
    ...
    results = {
        ...
        'decision_functions': all_decision_functions,
...
def compare_hemispheres(left_results, right_results, output_dir, logger):
    """Compare left and right hemisphere results for OvO models."""
    
    logger.info(f"\n{'='*80}")
    logger.info("COMPARING HEMISPHERES (ONE-VS-ONE)")
    logger.info(f"{'='*80}\n")
    
    comparison_dir = output_dir / "comparison_ovo"
...
def main():
    ...
    logger.info("="*80)
    logger.info("HEMISPHERE-SPECIFIC CLASSIFICATION (ONE-VS-ONE)")
    logger.info("Preprocessing: Diagonal imputation + Fisher Z → StandardScaler")
    logger.info("Model: One-vs-One (OvO) - Pairwise binary classifiers")
    ...
    try:
        ...
        logger.info("\n" + "="*80)
        logger.info("TRAINING COMPLETED SUCCESSFULLY (ONE-VS-ONE)")
        logger.info("="*80)
