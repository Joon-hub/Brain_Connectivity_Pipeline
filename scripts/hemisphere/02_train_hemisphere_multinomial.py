...
Train multinomial logistic regression for hemisphere-specific classification.

Workflow: Load Data → Diagonal Imputation + Fisher Z → CV with fold-wise Optuna → Task Testing

Usage:
    python scripts/hemisphere/02_train_hemisphere_multinomial.py --hemisphere left --tune_hyperparams --test_on_task
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Train hemisphere-specific multinomial logistic regression')
    
    # Core settings
    parser.add_argument('--hemisphere', type=str, required=True, choices=['left', 'right', 'both'])
    parser.add_argument('--data_dir', type=Path, default=project_root / 'data' / 'processed' / 'hemispheres')
    parser.add_argument('--output_dir', type=Path, default=project_root / 'data' / 'results' / 'hemisphere_analysis')    
    # Model settings
...
def get_default_config() -> dict:
    ...
    return {
        'preprocessing': {'apply_fisher_z': True, 'standardize': True, 'diagonal_strategy': 'region_mean'},
        'model': {'solver': 'lbfgs', 'max_iter': 1000, 'multi_class': 'multinomial'}
    }
...
def sample_first_n_subjects(data: dict, n_sample: int, logger: logging.Logger) -> dict:
    ...
    logger.info(f"Selected: {', '.join(map(str, data['subject_ids'][:min(10, n_sample)]))}" + 
...
def test_on_task_data(hemisphere: str, best_params: dict, random_state: int, n_jobs: int,
                     diagonal_strategy: str, data_dir: Path, output_dir: Path,
                     sample: int, logger: logging.Logger) -> dict:
    ...
    # Load & preprocess REST data
    logger.info("Loading rest data...")
    rest_data = load_hemisphere_data(data_dir, hemisphere, 'rest', return_matrix=True, validate=True)
    if sample: rest_data = sample_first_n_subjects(rest_data, sample, logger)
    logger.info(f"  Rest: {rest_data['n_subjects']} subjects, {rest_data['n_regions']} regions")
    
    rest_connectivity = apply_diagonal_imputation(rest_data['connectivity'], rest_data['region_info'], diagonal_strategy, logger)
    rest_connectivity = apply_fisher_z_transformation(rest_connectivity, logger)
    X_rest, y_rest, groups_rest = prepare_classification_data(rest_connectivity, rest_data['region_info'], rest_data['subject_ids'])
    
    # Load & preprocess TASK data
    logger.info("\nLoading task data...")
    task_data = load_hemisphere_data(data_dir, hemisphere, 'task', return_matrix=True, validate=True)
    if sample: task_data = sample_first_n_subjects(task_data, sample, logger)
    logger.info(f"  Task: {task_data['n_subjects']} subjects, {task_data['n_regions']} regions")
    
    task_connectivity = apply_diagonal_imputation(task_data['connectivity'], task_data['region_info'], diagonal_strategy, logger)
    task_connectivity = apply_fisher_z_transformation(task_connectivity, logger)
    X_task, y_task, groups_task = prepare_classification_data(task_connectivity, task_data['region_info'], task_data['subject_ids'])
    ...
    task_per_region = compute_per_region_metrics(y_task, y_task_pred, task_data['region_info'])
    task_network = compute_network_level_metrics(y_task, y_task_pred, task_data['region_info'])
...
def train_single_hemisphere(hemisphere: str, args: argparse.Namespace, 
                           logger: logging.Logger, optuna_n_jobs: int) -> dict:
    ...
    data = load_hemisphere_data(args.data_dir, hemisphere, 'rest', return_matrix=True, validate=True)
    if args.sample:
        data = sample_first_n_subjects(data, args.sample, logger)
        logger.warning(f"⚠️  TESTING MODE: {args.sample} subjects only")
    
    connectivity, subject_ids, region_info = data['connectivity'], data['subject_ids'], data['region_info']
    n_subjects, n_regions, _ = connectivity.shape
    ...
    # Report
    logger.info(f"\n{'='*80}")
    logger.info(f"OVERALL CV RESULTS ({hemisphere.upper()})")
    logger.info(f"{'='*80}")
    ...
    if args.test_on_task:
        ...
        task_results = test_on_task_data(
            hemisphere, best_params_for_task, args.random_state, args.n_jobs,
            args.diagonal_strategy, args.data_dir, args.output_dir, args.sample, logger
        )
    
    return {
        'hemisphere': hemisphere, 'n_subjects': n_subjects, 'n_regions': n_regions,
...
def compare_hemispheres(left_results, right_results, output_dir, logger):
    ...
    left_acc, right_acc = left_results['overall_metrics']['accuracy'], right_results['overall_metrics']['accuracy']
    ...
    summary = {
        'left_accuracy': left_acc, 'right_accuracy': right_acc,
        'difference': abs(left_acc - right_acc), 'ttest_p_value': float(p_value)
    }
    ...
    if left_results.get('task_results') and right_results.get('task_results'):
        ...
        summary.update({
            'left_task_accuracy': left_task, 'right_task_accuracy': right_task,
            'left_accuracy_drop': left_results['task_results']['task_summary']['accuracy_drop'],
            'right_accuracy_drop': right_results['task_results']['task_summary']['accuracy_drop']
        })
        logger.info(f"\nTask: Left={left_task:.4f}, Right={right_task:.4f}")
        logger.info(f"Drops: Left={summary['left_accuracy_drop']:.4f}, Right={summary['right_accuracy_drop']:.4f}")
    
    if left_results.get('fold_best_params') and right_results.get('fold_best_params'):
        left_ref, right_ref = left_results['reference_params'], right_results['reference_params']
        logger.info(f"\nHyperparams:")
        logger.info(f"  Left:  C={left_ref['C']:.6f} (range: [{left_ref['_C_range'][0]:.6f}, {left_ref['_C_range'][1]:.6f}])")
        logger.info(f"  Right: C={right_ref['C']:.6f} (range: [{right_ref['_C_range'][0]:.6f}, {right_ref['_C_range'][1]:.6f}])")
        summary.update({'left_hyperparameters': left_ref, 'right_hyperparameters': right_ref})
...
def main():
    ...
    logger.info("="*80)
    logger.info("HEMISPHERE MULTINOMIAL CLASSIFICATION")
    logger.info("="*80)
    if args.tune_hyperparams:
    ...
    try:
        if args.hemisphere == 'both':
            left = train_single_hemisphere('left', args, logger, optuna_n_jobs)
            right = train_single_hemisphere('right', args, logger, optuna_n_jobs)
            compare_hemispheres(left, right, args.output_dir, logger)
            
            logger.info("\n" + "="*80)
            logger.info("COMPLETED (BOTH HEMISPHERES)")
            logger.info("="*80)
            logger.info(f"Left:  CV={left['overall_metrics']['accuracy']:.4f}, Top-5={left['overall_metrics'].get('top_5_accuracy', 'N/A')}")
            logger.info(f"Right: CV={right['overall_metrics']['accuracy']:.4f}, Top-5={right['overall_metrics'].get('top_5_accuracy', 'N/A')}")
        else:
