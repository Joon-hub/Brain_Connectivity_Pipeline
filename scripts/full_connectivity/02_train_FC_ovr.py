...
Train One-vs-Rest (OvR) logistic regression on FULL brain connectivity (232 regions).
This establishes baseline performance using binary classification approach where each
region is classified against all other regions.
...
Usage:
    python scripts/full_connectivity/03_train_fC_ovr.py
    python scripts/full_connectivity/03_train_fC_ovr.py --tune_hyperparams
    python scripts/full_connectivity/03_train_fC_ovr.py --test_on_task
    python scripts/full_connectivity/03_train_fC_ovr.py --tune_hyperparams --test_on_task
    
    # With sampling for testing:
    python scripts/full_connectivity/03_train_fC_ovr.py --sample 30
"""
...
def setup_logging(output_dir: Path) -> logging.Logger:
    ...
    log_file = output_dir / "training_full_connectivity_ovr.log"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Train full connectivity One-vs-Rest logistic regression (232 regions)'
    )
...
def optimize_hyperparameters_optuna(
    ...
    random_state : int
        Random state
    n_jobs : int
        Number of parallel jobs for OvR
    logger : logging.Logger
        Logger instance
...
def test_on_task_data(
    ...
    logger.info(f"  OvR model trained in {train_time:.2f}s")
    logger.info(f"  Number of binary classifiers: {len(final_model.estimators_)}")
    ...
    task_output_dir = output_dir / "task_testing_ovr"
    ...
    task_summary = {
        ...
        'n_binary_classifiers': int(len(final_model.estimators_))
    }
...
def train_full_connectivity_ovr(args: argparse.Namespace, logger: logging.Logger) -> dict:
    """
    Train One-vs-Rest model for full brain connectivity (232 regions).
    ...
    8. [If test_on_task] Train final OvR model on all rest data → Test on task data
    """
    
    logger.info(f"\n{'='*80}")
    logger.info(f"TRAINING FULL BRAIN CONNECTIVITY (232 REGIONS) - ONE-VS-REST")
    logger.info(f"{'='*80}\n")
    
    # Create output directory
    output_dir = args.output_dir / "ovr"
    ...
    logger.info(f"  Binary classifiers needed: {n_classes} (One-vs-Rest)")
    ...
    logger.info(f"Note: Training {n_classes} binary classifiers per fold\n")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(gkf.split(X, y, groups=groups)):
        ...
        # Train OvR model
        logger.info(f"  Training OvR with C={C:.6f}, solver={solver}...")
        ...
        fold_metric_dict = {
            ...
            'n_binary_classifiers': len(model.estimators_),
            'hyperparameters': {
                'C': float(C),
                'max_iter': int(max_iter),
                'solver': solver,
        ...
        logger.info(f"  Binary classifiers trained: {len(model.estimators_)}")
    ...
    overall_metrics['model_type'] = 'one_vs_rest'
    overall_metrics['n_binary_classifiers'] = n_classes
    
    logger.info(f"\nOVERALL RESULTS (FULL CONNECTIVITY - {n_regions} REGIONS - ONE-VS-REST):")
    ...
    plot_confusion_matrix(
        ...
        title='Full Connectivity (232 Regions) - Confusion Matrix (OvR)'
    )
    
    plot_per_region_accuracy(
        per_region_metrics=per_region_metrics,
        save_path=output_dir / 'per_region_accuracy.png',
        title='Full Connectivity (232 Regions) - Per-Region Accuracy (OvR)'
    )
    
    plot_network_accuracy(
        network_metrics=network_metrics,
        save_path=output_dir / 'network_accuracy.png',
        title='Full Connectivity (232 Regions) - Network-Level Accuracy (OvR)'
    )
    ...
    results = {
        ...
        'n_classes': n_classes,
        'n_samples': n_samples,
        'n_binary_classifiers': n_classes,
...
def main():
    ...
    logger.info("="*80)
    logger.info("FULL BRAIN CONNECTIVITY CLASSIFICATION (ONE-VS-REST)")
    logger.info("232 Regions | 224 Subjects (PIOP2 Rest) | N Binary Classifiers")
    logger.info("="*80)
    logger.info("Preprocessing: Diagonal imputation + Fisher Z → StandardScaler")
    logger.info("Model: One-vs-Rest (OvR) - 232 binary classifiers")
    ...
    try:
        results = train_full_connectivity_ovr(args, logger)
        
        logger.info("\n" + "="*80)
        logger.info("TRAINING COMPLETED SUCCESSFULLY (ONE-VS-REST)")
        logger.info("="*80)
        logger.info(f"\nFinal Results:")
        ...
        logger.info(f"  • Binary Classifiers: {results['n_binary_classifiers']}")
