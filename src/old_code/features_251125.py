...
class FoldAwareDiagonalImputer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.strategy == "random":
            R = X.shape[1]
            for s in range(n_subjects):
                # Fresh random values for each transform call
                vals = np.random.uniform(-1, 1, size=R)
                np.fill_diagonal(X_imputed[s], vals)
            return X_imputed
        ...
        if self.strategy == "sample_from_matrix":
            ...
            for s in range(n_subjects):
                M = X_imputed[s]
                for r in range(R):
                    # Sample from off-diagonal values in this row
                    candidates = M[r, mask[r]]
                    candidates = candidates[~np.isnan(candidates)]
                    
                    if len(candidates) > 0:
                        # Fresh random choice for each transform
